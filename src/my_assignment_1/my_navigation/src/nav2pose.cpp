...
public:
    ...
    Nav2ActionClient() : Node("nav2pose")
    {
    ...
    void send_goal()
    {
        ...
        while (!tf_buffer_->canTransform("map", "tag36h11:1", tf2::TimePointZero)) {
            RCLCPP_WARN(this->get_logger(), "Transform not available yet...");
            rclcpp::sleep_for(std::chrono::milliseconds(200));
        }

        try{tf_apriltag0_wrt_map = tf_buffer_->lookupTransform("map", "tag36h11:1", tf2::TimePointZero);} 
        catch(const tf2::TransformException & ex){
            RCLCPP_INFO(this->get_logger(), "Could not transform %s to %s: %s", "tag36h11:1", "map", ex.what());
            return;
        }
        ...
        try{tf_apriltag1_wrt_map = tf_buffer_->lookupTransform("map", "tag36h11:10", tf2::TimePointZero);} 
        catch(const tf2::TransformException & ex){
            RCLCPP_INFO(this->get_logger(), "Could not transform %s to %s: %s", "tag36h11:10", "map", ex.what());
            return;
        }
...
private:
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Received feedback. Distance_remaining = %.2f", feedback->distance_remaining);
    }
